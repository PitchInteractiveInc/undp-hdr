...
export default function GSNIGraphWrapper(props) {
  ...
  return <GSNIGraph {...props} gsniData={gsniData} />
}
function GSNIGraph(props) {
  ...
  const yMax = max(countries, d => d[gsniKey])
  // console.log(yMax)
  ...
  const countryBars = sortedCountries.map((country, countryIndex) => {
    const totalBarHeight = yScale(country[gsniKey])
    ...
    delaunayData.push([x, height / 2, {row: country, col: gsniKey}])
    let label = null
    if (isSelected) {
      label = (
  ...
  return (
    <div className='IndexGraph'>
      ...
      <div style={{ fontWeight: 'bold', marginBottom: '0.5em'}}>
        Contribution of deprivation in indicator to overall GSNI
      </div>
