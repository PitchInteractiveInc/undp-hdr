...
export default function BarGraph(props) {
  ...
  if (index.key === 'GSNI' || index.key == 'MPI') {
    console.log(index.key, data, graphColumns)
    console.log(data.columns.filter(d => d.includes('mpi')))
  }
  ...
  sortedData.sort((a, b) => {
    ...
    return index.key === 'MPI' || index.key === 'GSNI' ? bValue - aValue : aValue - bValue
  })
  ...
  const bars = sortedData.map((country, i) => {
    ...
    let label = showLabel ? <text x={x} key={i} dy='-0.5em' textAnchor='middle' fill={labelFill} y={height - y}>{format(value, index.key)}</text> : null
    labels.push(label)
    return (
      <g transform={`translate(${x}, ${0})`} key={i}>
        <Rect
          width={barWidth}
          y={isNaN(height - y) ? 0 : height - y}
          height={isNaN(y) ? 0 : y}
          fill={fill}
          stroke={stroke}
          index={i}
  ...
  const yScaleTicks = yScale.ticks(10).map((tick) => {
    const y = yScale(tick)
    return (
      <g key={tick} transform={`translate(${width}, ${height - y})`}>
        <text dy='0.3em' dx='0.5em'>{format(tick, index.key)}</text>
